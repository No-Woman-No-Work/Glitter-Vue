...
export default {
  ...
  setup(props) {
    ...
    const search = async(currentSearch) => {
      if(!currentSearch.value) {
        return
      }
      try {
        const response = await flitterApi.get(`/tweets?search=${currentSearch}`)
        const data = response.data.docs
        if (data.length > 0) {
          tweets.value = data
          totalTweets.value = response.data.length
          console.table(data)
        }
      }catch(error) {
        console.error(error)
    ...
    watch(() => currentSearch.value, () => {	
      ...
      search(currentPage.value, defaultLimit, currentOrder.value);	
    })	
